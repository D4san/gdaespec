...
def main():
    """Main execution function for retrieval with stellar contamination."""
    t_start = time.time()
    ...
    # Create the stellar object with a two-spots contamination model
    star = create_star(
    ...
    # Load observational data (contaminated case)
    data_dir = Path("observations")
    obs_file = "pandexo_output_100transits.dat"
    datasets = [obs_file]
    ...
    # --- Model Definition ---
    # Model configuration for Contam strategy (includes stellar contamination)
    model_name = "contam_100T_0.00spot-0.00fac"
    bulk_species = ["N2"]
